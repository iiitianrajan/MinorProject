...
import {
  ...
  ArrowRight, ChevronRight, Filter, Search, Flame,
  Shield, Gift, Users, Calendar
} from 'lucide-react';

/* ─── Section Label (copied from Home) ─── */
...
const CategoryPill = ({ label, active, onClick }) => (
  <motion.button
    ...
    className={`px-5 py-2 rounded-full text-xs font-semibold uppercase tracking-widest border transition-all duration-300 cursor-pointer whitespace-nowrap ${
      active
        ? 'bg-[var(--gradient-primary)] text-white border-transparent shadow-[0_4px_18px_rgba(255,98,0,0.28)]'
...
export default function Puja() {
  ...
  useEffect(() => {
    const fetchPujas = async () => {
      try {
        ...
        const cats = ['ALL', ...new Set(data.map(p => p.category?.toUpperCase()).filter(Boolean))];
        setCategories(cats);
      } catch (err) {
        console.error('❌ Puja fetch error:', err);
        // Fallback demo data
        const demo = [
          { _id: '1', name: 'Mahalakshmi Abundance Puja', category: 'PROSPERITY', description: 'Invoke the goddess of wealth to invite spiritual and material prosperity into your life.', duration: 45, price: 169, rating: 4.9, image: 'https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=600&q=80' },
          { _id: '2', name: 'Navagraha Shanti Ritual', category: 'ALIGNMENT', description: 'Harmonize planetary energies to remove cosmic friction from your destiny path.', duration: 60, price: 149, rating: 4.8, image: 'https://images.unsplash.com/photo-1560472355-536de3962603?w=600&q=80' },
          { _id: '3', name: 'Ganesha Vighnaharta Puja', category: 'PROTECTION', description: 'The ultimate ritual for removing obstacles and ensuring success in new ventures.', duration: 30, price: 75, rating: 5.0, image: 'https://images.unsplash.com/photo-1600880292203-757bb62b4baf?w=600&q=80' },
          { _id: '4', name: 'Satyanarayan Katha', category: 'PROSPERITY', description: 'A sacred offering to Lord Vishnu for truth, peace and prosperity in family life.', duration: 90, price: 249, rating: 4.7, image: 'https://images.unsplash.com/photo-1559827260-dc66d52bef19?w=600&q=80' },
          { _id: '5', name: 'Rudra Abhishek', category: 'ALIGNMENT', description: 'A powerful Shiva ritual to dispel negative karma and invite divine blessings.', duration: 60, price: 199, rating: 4.9, image: 'https://images.unsplash.com/photo-1544551763-46a013bb70d5?w=600&q=80' },
          { _id: '6', name: 'Durga Saptashati Path', category: 'PROTECTION', description: 'Invoke the divine mother for courage, protection and victory over adversity.', duration: 120, price: 299, rating: 5.0, image: 'https://images.unsplash.com/photo-1605810230434-7631ac76ec81?w=600&q=80' },
        ];
        setPujas(demo);
  ...
  const filtered = pujas.filter(p => {
    const matchCat = activeCategory === 'ALL' || p.category?.toUpperCase() === activeCategory;
          ...
          <div className="max-w-2xl">
            ...
            >
              ...
              <motion.span
                ...
                style={{
                  background: 'linear-gradient(90deg, #ff8c3a, #ff6200, #ffad75, #ff6200)',
      ...
      <section id="puja-list" className="section">
        <PageContainer>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-10">
            ...
            >
              <Search size={15} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-soft)]" />
              <input
                type="text"
                placeholder="Search rituals…"
                value={search}
                onChange={e => setSearch(e.target.value)}
                className="w-full pl-10 pr-4 py-3 rounded-full text-sm border border-[var(--border-soft)] bg-[var(--bg-elevated)] text-[var(--text)] placeholder:text-[var(--text-soft)] outline-none focus:border-[var(--accent-border)] transition-all duration-200"
              />
            </motion.div>
          ...
          {loading ? (
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center py-20 card-soft"
            >
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 max-w-md">
                <p className="text-xs font-semibold uppercase tracking-widest mb-4" style={{ color: '#ff8c3a' }}>
                  How It Works
                </p>
                <h2 className="text-3xl lg:text-4xl font-bold leading-tight mb-10 tracking-tight" style={{ color: '#f5f5f5' }}>
                  The Sacred Process
                </h2>
            ...
            >
              {['First ritual blessing free', 'Certified Vedic pandits', '100% authentic rituals'].map((txt, i) => (
                <motion.div
                  key={i}
                  whileHover={{ scale: 1.06, y: -2 }}
                  className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs cursor-default bg-[var(--accent-bg)] border border-[var(--accent-border)] text-[var(--text-muted)]"
                >
